...
sub get_user {
	my ($id) = @_;
	my $user = $db->get_user($id) or return 404,'{}';
	return 200, $user;
}

sub get_visit {
	my ($id) = @_;
	my $visit = $db->get_visit($id) or return 404,'{}';
	return 200, $visit;
}

sub get_location {
	my ($id) = @_;
	my $loc = $db->get_location($id) or return 404,'{}';
	return 200, $loc;
}
...
my $server = Local::HTTPServer->new("0.0.0.0",$port, sub {
	my ($met,$path,$query,$body) = @_;
	# p @_;
	if ($met eq 'GET') {
		my ($m,$cap) = $GET->match($path);
		++$REQS;
		$m or return 404, '{"error":"path"}';
		$m->( $cap->{id}, $query );
	}
	elsif($met eq 'POST') {
